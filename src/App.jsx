...
import './App.css'
import bcgVideo from './videos/bcg-loop.mp4'
...
function App() {
  ...
  const [nameOfJuri, setNameOfJuri] = useState('Jury');
  ...
  const [bcgVideoLoop, setBcgVideoLoop] = useState('');
  ...
  const callData = async () => {
    const scoreArray = (await axios.get('https://us-central1-api-tvef-vote.cloudfunctions.net/app/edition9')).data;
    setScoreBoard(scoreArray.final);

    const allJuri = (await axios.get('https://us-central1-api-tvef-vote.cloudfunctions.net/app/readPuans')).data;
    setJuri(allJuri);


    const arrayOfParticipants = (await axios.get('https://us-central1-api-tvef-vote.cloudfunctions.net/app/edition9')).data;
    setAllCountries(arrayOfParticipants.final);
  ...
  const moveBox = () => {
    if (moveCount < 0) {
      ...
      setTeleCount(t + 1);
      setBcgVideoLoop('src/videos/bcg-loop.mp4');
    }
  ...
  return (
    <>
      {
        window.innerWidth < 900 ? <div>This program is only sutible for computers</div> : <>
          <div className={`back ${classOfBack}`}>
            <video src={`${bcgVideo}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} autoPlay loop muted></video>
          </div>
          <section className='main'>
            ...
            <div className='juri-tele'>
              <video src='./videos/and.mp4' autoplay=""></video>
              <div>
                <p>
